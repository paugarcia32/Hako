...
export class YoutubeScraperService implements IScraper {
  ...
  async scrape(url: string): Promise<ScrapeResult> {
    ...
    // Returns title, author_name (channel), thumbnail_url reliably even when
    // YouTube blocks generic HTML scrapers.
    try {
    ...
    // Phase 2: Full HTML fallback — handles edge cases (unlisted videos,
    // Shorts, embeds) where oEmbed may be unavailable.
    try {
      const html = await this.utils.fetchHtml(url);
      if (html) {
